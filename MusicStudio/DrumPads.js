...
DrumSample.prototype.play = function(pad) {
	...
	if(pad==1){
		...
		this.wood.gainNode.gain.value = this.vol1 * this.volume;
		this.wood.source.playbackRate.value = Math.pow(2, (this.pitch1/12));
		this.wood.source.start(0);
	}
	if(pad==2){
		this.clav = createSource(this.pad2);
		this.clav.gainNode.gain.value = this.vol2 * this.volume;
		this.clav.source.playbackRate.value = Math.pow(2, (this.pitch2/12));
		this.clav.source.start(0);
	}
	if(pad==3){
		this.cym1 = createSource(this.pad3);
		this.cym1.gainNode.gain.value = this.vol3 * this.volume;
		this.cym1.source.playbackRate.value = Math.pow(2, (this.pitch3/12));
		this.cym1.source.start(0);
	}
	if(pad==4){
		this.vox1 = createSource(this.pad4);
		this.vox1.gainNode.gain.value = this.vol4 * this.volume;
		this.vox1.source.playbackRate.value = Math.pow(2, (this.pitch4/12));
		this.vox1.source.start(0);
	}
	
	if(pad==5){
		this.tamb = createSource(this.padq);
		this.tamb.gainNode.gain.value = this.vol5 * this.volume;
		this.tamb.source.playbackRate.value = Math.pow(2, (this.pitch5/12));
		this.tamb.source.start(0);
	}
	if(pad==6){
		this.hat3 = createSource(this.padw);
		this.hat3.gainNode.gain.value = this.vol6 * this.volume;
		this.hat3.source.playbackRate.value = Math.pow(2, (this.pitch6/12));
		this.hat3.source.start(0);
	}
	if(pad==7){
		this.hat2 = createSource(this.pade);
		this.hat2.gainNode.gain.value = this.vol7 * this.volume;
		this.hat2.source.playbackRate.value = Math.pow(2, (this.pitch7/12));
		this.hat2.source.start(0);
	}
	if(pad==8){
		this.tom1 = createSource(this.padr);
		this.tom1.gainNode.gain.value = this.vol8 * this.volume;
		this.tom1.source.playbackRate.value = Math.pow(2, (this.pitch8/12));
		this.tom1.source.start(0);
	}
	
	if(pad==9){
		this.snr3 = createSource(this.pada);
		this.snr3.gainNode.gain.value = this.vol9 * this.volume;
		this.snr3.source.playbackRate.value = Math.pow(2, (this.pitch9/12));
		this.snr3.source.start(0);
	}
	if(pad==10){
		this.hat1 = createSource(this.pads);
		this.hat1.gainNode.gain.value = this.vol10 * this.volume;
		this.hat1.source.playbackRate.value = Math.pow(2, (this.pitch10/12));
		this.hat1.source.start(0);
	}
	if(pad==11){
		this.snr2 = createSource(this.padd);
		this.snr2.gainNode.gain.value = this.vol11 * this.volume;
		this.snr2.source.playbackRate.value = Math.pow(2, (this.pitch11/12));
		this.snr2.source.start(0);
	}
	if(pad==12){
		this.snr1 = createSource(this.padf);
		this.snr1.gainNode.gain.value = this.vol12 * this.volume;
		this.snr1.source.playbackRate.value = Math.pow(2, (this.pitch12/12));
		this.snr1.source.start(0);
	}
	
	if(pad==13){
		this.kick1 = createSource(this.padz);
		this.kick1.gainNode.gain.value = this.vol13 * this.volume;
		this.kick1.source.playbackRate.value = Math.pow(2, (this.pitch13/12));
		this.kick1.source.start(0);
	}
	if(pad==14){
		this.kick2 = createSource(this.padx);
		this.kick2.gainNode.gain.value = this.vol14 * this.volume;
		this.kick2.source.playbackRate.value = Math.pow(2, (this.pitch14/12));
		this.kick2.source.start(0);
	}
	if(pad==15){
		this.tr808 = createSource(this.padc);
		this.tr808.gainNode.gain.value = this.vol15 * this.volume;
		this.tr808.source.playbackRate.value = Math.pow(2, (this.pitch15/12));
		this.tr808.source.start(0);
	}
	if(pad==16){
		this.fx1 = createSource(this.padv);
		this.fx1.gainNode.gain.value = this.vol16 * this.volume;
		this.fx1.source.playbackRate.value = Math.pow(2, (this.pitch16/12));
...
DrumSample.prototype.setMasterVol = function(element){
	this.volume = parseFloat(element.value);
}